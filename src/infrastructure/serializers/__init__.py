    ...
    def serialize(cls, dataset: TrackingDataset) -> str:
        def frame_to_dict(frame: Frame):
            return dict(
                ...
                home=[{'x': player.x, 'y': player.y} for player in frame.home_players_coordinates.values()],
                away=[{'x': player.x, 'y': player.y} for player in frame.away_players_coordinates.values()]
            )

        return ujson.dumps(
            [frame_to_dict(frame) for frame in dataset.frames]
        )
